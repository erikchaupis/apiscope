import { CollectionRequest, Endpoint, HttpMethod } from './types';
...
export function resolveEndpointDisplayName(endpoint: Endpoint): string {
...
export function isLegacyMethodPathName(request: CollectionRequest): boolean {
  const name = request.name?.trim();
  return Boolean(name && name === `${request.method} ${request.path}`);
}
